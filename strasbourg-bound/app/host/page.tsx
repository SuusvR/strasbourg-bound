...
function computeAwardWinners() {
  ...
  const photoUrl = `${window.location.origin}/player?code=${roomCode}`
          ...
          {revealed && sortedTally.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {sortedTally.map(([name, count]) => (
                <div key={name} className={name === winner ? 'card' : ''} style={{ background: name === winner ? 'rgba(232,184,109,0.08)' : 'var(--bg3)', border: name === winner ? '1px solid rgba(232,184,109,0.4)' : '1px solid var(--border)', borderRadius: 12, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
